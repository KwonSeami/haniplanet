...
import {USER_TYPE_TO_KOR, MAIN_USER_TYPES} from '../../../src/constants/users';
...
const BandMemberItem: React.FC<IBandMemberItem> = React.memo((
  ...
  const {query: {slug, ...search}} = useRouter();
  const [
    korName,
    engName,
  ...
  return (
    <li>
      <div onClick={() => setIsOpened(curr => !curr)}>
        <img
          src={avatarExposeType(user_expose_type, user.avatar)}
          alt="프로필 이미지"
        />
        <h3>{engName}</h3>
        <p>{`가입완료일 ${toDateFormat(approved_at, 'YYYY.MM.DD')} · 가입신청일 ${toDateFormat(created_at, 'YYYY.MM.DD')}`}</p>
  ...
  return (
    ADMIN_PERMISSION_GRADE.includes(band_member_grade) ? (
      ...
      <TitleDiv avatar={avatar}>
        ...
        <h2>
          <Link href="/onclass/[slug]" as={`/onclass/${slug}`}>
            <a>
              <span>온라인 강의명</span>
              {name}
            </a>
