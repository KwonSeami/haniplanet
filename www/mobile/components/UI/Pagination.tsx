...
const getStartEnd = (current: number, maximum: number, groupSize = 5) => {
    ...
    <Div className={cn('pagination', className)}>
      {(currentPage > 1 && lastPage > pageGroupSize) && (
        <>
          ...
          >
            {paginationContentButton('icon-arrow-first1', '처음으로')}
          </button>
          <button
            onClick={() => onClick(prevGroup)}
          >
            {paginationContentButton('icon-arrow-prev1', '이전')}
          </button>
        </>
      )}
      <ul>
      ...
      {(currentPage < lastPage && lastPage > pageGroupSize) && (
        <>
          ...
          >
            {paginationContentButton('icon-arrow-next1', '다음')}
          </button>
          <button
            onClick={() => onClick(lastPage)}
          >
            {paginationContentButton('icon-arrow-last1', '마지막으로')}
          </button>
        </>
      )}
    </Div>
