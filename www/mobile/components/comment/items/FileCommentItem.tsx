...
import {$BORDER_COLOR, $FONT_COLOR, $GRAY, $TEXT_GRAY, $POINT_BLUE} from "../../../styles/variables.types";
...
const StyledAnchorSpan = styled.span`
	...
	${ props => {
		if( props.is_open ) return `color: ${$POINT_BLUE}`;
	}}
`;
        ...
        {files.map(({id, original_name, name, file, size}) => (
          <li className="file-item" key={id}>
            <A to={file} download newTab>
              <span className="filename">{original_name || name}</span>
...
const FileCommentItem = React.memo<IDefaultCommentItemProps>((
  ...
  return (
    <CommentItemLi className="comment-item">
      <div>
        <h2>
          ...
          { !! files.length && (
          <span>
            <StyledAnchorSpan is_open={isOpen} onClick={() =>setOpen(curr => !curr)}>
