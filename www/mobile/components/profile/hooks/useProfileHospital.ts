...
const useProfileHospital = (id: HashId) => {
  ...
  const acFetch = React.useCallback((query: string) => {
    if (!query) return null;

    searchApi.hospitalAutoComplete(query, {band_limit: 5})
      .then(({status, data: {result}}) => {
        const {band} = result;
        setAcList(status === 200 ? band : []);
      });
  }, []);

  const throttledACFetch = React.useCallback(throttle(acFetch, 500), [acFetch]);

  const searchHospital = React.useCallback((query: string) => {
    if (query.length >= 2) {
      router.push({
        pathname,
        query: {tab: 'hospital', query},
      });
    } else {
