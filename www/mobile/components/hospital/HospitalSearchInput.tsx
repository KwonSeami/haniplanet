...
const MAX_AUTO_COMPLETE_LENGTH = 5;
...
const HospitalSearchInput = React.memo<Props>(({
  ...
  const getReleatedSearch = React.useCallback((query: string) => {
    ...
    callNaverGeocode(query)
      .then(({v2: {addresses}}) => {
        searchApi.hospitalAutoComplete(query)
          .then(({data: {result}}) => {
            const naverAddresses = addresses.map(({roadAddress, jibunAddress, x, y}) => ({
              x,
              y,
              type: 'location',
              name: roadAddress || jibunAddress,
            }));
            const {band, category} = result;
  ...
  const onSearchKeyword = React.useCallback((query: string) => {
    ...
    let queryParams = {q: query} as any;
