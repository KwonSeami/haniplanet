...
const Professor = React.memo(() => {
  ...
  const {asPath} = router;
  const {tag: _tagQuery, q: _qQuery, order_by: _orderByQuery} = router.query;
  ...
  const {count, access} = useSelector(
          ...
          <AdditionalContent>
            <SearchInput
              ...
              onChange={({target: {value}}) => {
                setQ(value);
                throttledSetFetchURI(`${FETCH_URL}?${queryString.stringify({
                  q: value,
                  tag: _tagQuery,
                  order_by: _orderByQuery
                })}`);
              ...
              {!isEmpty(professorCountList) ? (
                professorCountList.map(({name, stories_count}) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        as={name === "전체" ? '/professor' : `/professor?${query}`}
                        href={name === "전체" ? '/professor' : `/professor?${query}`}
                        passHref
                        replace
                      >
                        <a>
