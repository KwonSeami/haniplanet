...
const useBandEdit = (slug: string) => {
  ...
  const isValidForm = React.useCallback((currState: IEditForm, defaultState: IEditForm): [
    ...
    const {purpose, body, questions: {first, second}} = currState;
    const {
      purpose: _purpose,
      body: _body,
      questions: {first: _first, second: _second}
    } = defaultState;
    ...
    if (first.question !== _first.question) {
      if (!first.question) {
    ...
    if (second.question !== _second.question) {
      if (!second.question) {
