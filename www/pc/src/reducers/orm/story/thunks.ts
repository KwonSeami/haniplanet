...
export const fetchShopThunk = (storyPk: HashId, duration = 30 * MINUTE) =>
  (dispatch, getState) => {
    const {system: {session: {access}}, orm} = getState();
    const {retrieved_at} = pickStorySelector(storyPk)(orm) || {} as any;
    if (!retrieved_at || timeOver(retrieved_at, duration)) {
      new Api({token: access, baseURL: BASE_URL}).getAxios().get(`/shopping/goods/${storyPk}/`)
        .then(({status, data}) => {
          if (Math.floor(status / 100) === 4) {
            dispatch(errorOccuredStoryThunk(storyPk, {
              ...data,
            ...
            data && dispatch(saveStory({...data, retrieved_at: new Date().getTime()}));
          }
        })
        .catch(err => {
          dispatch(errorOccuredStoryThunk(storyPk, err.response));
...
export const toggleApplyStatusThunk = (storyPk: HashId, applyPk: HashId, form: Indexable) =>
  (dispatch, getState) => {
    ...
    api.patchApply(storyPk, applyPk, form)
      .then(({status}) => {
        if (Math.floor(status / 100) !== 4) {
          return dispatch(updateStory(storyPk, curr => ({
            ...curr,
            applies: curr.applies.map(apply => apply.id === applyPk ? {...apply, ...form} : apply),
          })));
        }
      });
  };
