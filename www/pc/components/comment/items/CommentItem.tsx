...
const CommentItem = React.memo<IDefaultCommentItemProps>((
  ...
  const {asPath, pathname, query:{slug, timeline: timelineId}} = useRouter();
  const isOnClass = pathname.includes('onclass');
  const dispatch = useDispatch();

  const {user, created_at, did_report, status, ...commentData} = item;
  ...
  const commentId = depth !== 0 ? `${parentPk}-${id}` : `${id}`;
  const onClassQA = useSelector(({orm}) => pickBandSelector(slug)(orm) || {}, shallowEqual);
  const {timelines} = onClassQA || {};
  ...
  return (
    <CommentItemLi
      className={cn('comment-item', {'onclass-answer': depth === 0 && (isOnClassQA && member_grade === 'admin')})}
    >
      <span
            ...
            <h2 className="user">
              ...
              {user && user.user_type && (
                <Label
                  ...
                  borderColor={user.user_type === 'doctor' ? '#7fc397' : '#9586d0'}
                />
              )}
              <span className="date">
