...
export const useTabParams = (paramsKey: string, tabList: string[]) => {
  ...
  const tabPrams = React.useMemo(() => queryString.parse(search)[paramsKey], [search, paramsKey]);
  ...
  const replaceTab = React.useCallback((tabValue) => {
    if (isValidTabParams(tabValue)) {
      router.replace(`${pathname}?${paramsKey}=${tabValue}`);
    }
  }, [isValidTabParams, pathname]);

  // 전달받은 `tabList`와 일치하지 않은 `params`를 갖고 있다면, `replace`시킵니다.
  React.useEffect(() => {
    if (!isValidTabParams(tabPrams)) {
      router.replace(`${pathname}?${paramsKey}=${tabList[0]}`);
    }
  ...
  }, [isValidTabParams, tabPrams, move]);
