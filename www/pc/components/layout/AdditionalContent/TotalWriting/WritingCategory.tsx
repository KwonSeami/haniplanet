...
const WritingCategory: React.FC<Props> = ({
  name,
  hasAdminPermission,
  editTimeline,
  deleteTimeline,
  ...
  const [{editing, category, isHover}, setState] = React.useState({
    category: name,
    isHover: false,
    editing: false,
  });
  ...
  return (
    editing ? (
      ...
      <Link
        href={{pathname, query: {timeline: id}}}
        as={{pathname: asPath.split('?')[0], query: {timeline: id}}}
        replace
        passHref
      >
        <a
