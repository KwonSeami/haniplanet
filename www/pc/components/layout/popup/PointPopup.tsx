...
const PointPopup = React.memo<Props>(
  ({id, closePop, storyPk}) => {
    ...
    React.useEffect(() => {
      // 스타일이 로딩되는 시간이 컴포넌트가 띄워지는 시간보다 늦어져 input에 입력시 popup이 이동되는 문제가 발생합니다.
      // 이를 개선하기 위해 스타일이 로딩되는 시간을 감안하여 150ms 이후에 popup의 toCenter 함수를 호출합니다.
      popupRef && setTimeout(() => {
        popupRef.current.toCenter();
      }, 150);
    }, [popupRef]);
        ...
        <SendPointDiv>
          ...
          <div className="payment-box">
            ...
            <ChargeButton
              ...
              onClick={() => {dispatch(pushPopup(PointChargePopup))}}
            >
              충전하기
            </ChargeButton>
