...
const MeetupPaymentTable: React.FC<Props> = ({
  ...
  // Hooks
  const filterProductsByUserType = React.useMemo(() => {
    if (!user_type) {
      return [];
    }
    ...
    return filteredList;
  }, [user_type, is_online_meetup, products, status]);
  ...
  const payPrice: number = React.useMemo(() => {
    if (isEmpty(filterProductsByUserType) || isEmpty(selectedOption)) {
      return 0;
    }
    const _payPrice = filterProductsByUserType.filter(({id}) => id === selectedOption.id)[0];
  ...
  }, [filterProductsByUserType, selectedOption]);
    ...
    <Section>
          ...
          <Table>
            <tr>
              ...
              <td className="payment-type">
                {isPrepareToPay ? (
                  !isEmpty(filterProductsByUserType)
                    ? (
                      <ul>
                        {(filterProductsByUserType || []).map(product => {
                          const {
                            id,
                            name,
                            text,
      ...
      {(isPrepareToPay || (!is_online_meetup && !isOfflineMeetupPayed)) ?
        <StyledButton
          ...
          onClick={() => {
            if (!(filterProductsByUserType || []).length) {
            ...
            if (!is_online_meetup) {
              if (confirm('참여자의 개인정보(이름, 연락처) 제 3자(개설자) 제공과 결제에 동의하시겠습니까?')) {
                const form = {products: [{id: selectedOption.id, quantity: 1}], answers: [...userAnswers]};
              ...
              if (!isEmpty(questions) && !!userAnswers.find(({answer}) => answer === '')) {
                ...
                dispatch(pushPopup(OnClassExtensionPopup, {
                  status: statusKor,
                  productList: filterProductsByUserType,
                  selectedOption,
                  answers: userAnswers,
                  pay_method,
                  isMeetup: true
